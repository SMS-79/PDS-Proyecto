...
class TableroTest {
    ...
    void setUp() throws IdentificadorTableroException {
    	propietario = new Usuario(new UsuarioId(12345L), "Pedro", "pedro@test.com", "1234");
        tablero = new Tablero(TableroId.random(), "Tablero Proyecto", propietario, "url-proyecto");
    }

    // --- Tests de Listas de Tareas ---
    ...
    @Test
    void eliminarLista_eliminaLaListaDelTablero() {
    ...
    @Test
    void obtenerLista_encuentraLaListaPorId() {
        ListaTareas lista = tablero.crearLista("DOING");

        Optional<ListaTareas> listaEncontrada = tablero.obtenerLista(lista.getId());

        assertTrue(listaEncontrada.isPresent());
        assertEquals("DOING", listaEncontrada.get().getTipo());
    }
    ...
    void crearTarjetaTarea_laAñadeALaListaCorrecta() {
        ...
        // Verificamos que se añadió a la lista
        ListaTareas listaActualizada = tablero.obtenerLista(lista.getId()).get();
        assertEquals(1, listaActualizada.getTarjetas().size());
        assertTrue(listaActualizada.getTarjetas().contains(tarjeta));
    }
    ...
    void crearTarjetaCheckList_laAñadeALaListaCorrecta() {
        ...
        ListaTareas listaActualizada = tablero.obtenerLista(lista.getId()).get();
        assertEquals(1, listaActualizada.getTarjetas().size());
        assertTrue(listaActualizada.getTarjetas().contains(tarjetaCL));
    }
    ...
    void interactuarConCheckList_aTravesDelTablero() {
        ...
        tarjetaCL.addItem(item1);

        ListaTareas listaActualizada = tablero.obtenerLista(lista.getId()).get();
        TarjetaCheckList tarjetaGuardada = (TarjetaCheckList) listaActualizada.getTarjetas().get(0);
        
        assertEquals(1, tarjetaGuardada.getItems().size());
        assertFalse(tarjetaGuardada.todosCompletados());

        item1.setCompletado(true);
        assertTrue(tarjetaGuardada.todosCompletados());
    }

    // Tests de Reglas de Negocio del Agregado 

    @Test
    void operarEnTableroBloqueado_lanzaExcepcion() {
        ListaTareas lista = tablero.crearLista("TODO");
